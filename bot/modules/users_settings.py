...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        text = (
            ...
            f"➲ <b>Image URL :</b> <code>{escape(attachment_url) if attachment_url else 'Not Set'}</code>\n\n"
            ...
            f"you can use the same image or a completely different one.\n\n"
        ...
        new_state = data[3] == "on"
        attachment_url = user_dict.get("attachment_url", "")
        if new_state and not attachment_url.strip():
            await query.answer(
                "⚠️ Set Attachment Image URL first before enabling!", show_alert=True
            )
            new_state = False
        else:
            await query.answer(
