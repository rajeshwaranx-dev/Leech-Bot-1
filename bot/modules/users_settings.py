...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        text = BotTheme(
            ...
            SAMPLE_VIDEO="ON" if sample_video else "OFF",
            SAMPLE_DURATION=f"{sample_duration}s",
            WATERMARK="ON" if watermark_on else "OFF",
            ATTACHMENT="ON" if attachment_on else "OFF",
        )
...
async def set_attachment_photo(client, message, pre_event, key, direct=False):
    ...
    same pattern as set_thumb() but stored under a separate key
    (attachment_image) so it's independent from Leech Thumbnail.
    When this is set, it takes priority over attachment_url at leech time.
    """
    user_id = message.from_user.id
    handler_dict[user_id] = False
    path = "Attachments/"
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        sample_buttons.ibutton("Back", f"userset {user_id} leech", "footer")
        ...
        rfunc = partial(update_user_settings, query, "sample_video", "leech")
        await sendMessage(
            message,
        ...
        att_buttons.ibutton("Back", f"userset {user_id} leech", "footer")
        ...
        rfunc = partial(update_user_settings, query, "attachment", "leech")
        await sendMessage(
            message,
            "📤 Send a photo to use as the Attachment image.\n"
            "<b>Timeout:</b> 60 Sec.",
