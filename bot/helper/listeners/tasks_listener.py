    ...
    async def onDownloadComplete(self):
        ...
        if self.isLeech:
            ...
            if watermark_enabled:
                ...
                LOGGER.info(f"Watermark: ON | Text: {watermark_text} | Duration: {watermark_duration}s | Path: {up_dir}")
                up_dir = await auto_watermark_leech(up_dir, watermark_enabled, watermark_text, watermark_duration)
            # ---- END WATERMARK ----
            size = await get_path_size(up_dir)
            for s in m_size:
                size = size - s
