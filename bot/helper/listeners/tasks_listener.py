...
from bot.helper.ext_utils.watermark import auto_watermark_leech
from bot.helper.ext_utils.attachment import auto_attachment_leech
from bot.helper.ext_utils.remove_audio import auto_remove_audio_leech
from bot.helper.ext_utils.auto_rename import auto_rename_leech
...
class MirrorLeechListener:
    ...
    async def onDownloadComplete(self):
        ...
        if self.isLeech:
            # ---- AUTO MERGE WEBSERIES ----
            merge_mode = user_dict.get("merge_mode", "off")
            if merge_mode != "off":
                LOGGER.info(f"Auto Merge Mode: {merge_mode} | Path: {up_dir}")
            ...
            # ---- END SAMPLE VIDEO ----
            # ---- WATERMARK ----
            watermark_enabled = user_dict.get("watermark", False)
            if watermark_enabled:
                ...
                LOGGER.info(f"Watermark: ON | Text: {watermark_text} | Duration: {watermark_duration}s | Size: {watermark_size}px | Path: {up_dir}")
                up_dir = await auto_watermark_leech(up_dir, watermark_enabled, watermark_text, watermark_duration, watermark_size)
            # ---- END WATERMARK ----
            # ---- REMOVE AUDIO ----
            remove_audio_enabled = user_dict.get("remove_audio", False)
            if remove_audio_enabled:
                remove_audio_langs = user_dict.get("remove_audio_langs", "")
                LOGGER.info(f"RemoveAudio: ON | Langs: {remove_audio_langs} | Path: {up_dir}")
                up_dir = await auto_remove_audio_leech(up_dir, remove_audio_enabled, remove_audio_langs)
            # ---- END REMOVE AUDIO ----
            # ---- AUTO RENAME ----
            auto_rename_enabled = user_dict.get("auto_rename", False)
            if auto_rename_enabled:
                rename_template = user_dict.get("auto_rename_template", "")
                LOGGER.info(f"AutoRename: ON | Template: {rename_template} | Path: {up_dir}")
                up_dir = await auto_rename_leech(up_dir, auto_rename_enabled, rename_template)
                up_name = up_dir.rsplit("/", 1)[-1]
            # ---- END AUTO RENAME ----
            # ---- ATTACHMENT ----
            attachment_enabled = user_dict.get("attachment", False)
            if attachment_enabled:
                attachment_url = user_dict.get("attachment_url", "")
                attachment_image = user_dict.get("attachment_image", "")
                LOGGER.info(f"Attachment: ON | Image: {attachment_image or 'None'} | URL: {attachment_url or 'None'} | Path: {up_dir}")
                up_dir = await auto_attachment_leech(up_dir, attachment_enabled, attachment_url, attachment_image)
            # ---- END ATTACHMENT ----
            size = await get_path_size(up_dir)
            for s in m_size:
                size = size - s
